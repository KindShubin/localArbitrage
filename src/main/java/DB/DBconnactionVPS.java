...
public class DBconnactionVPS {
    ...
    public DBconnactionVPS() throws SQLException {
        try {
...
//            // блок для отображения типов данных столбцов БД а также манипуляция с ними
            ...
            ex.printStackTrace();
            System.out.println("FAIL connection and statement in constructor DBconnect()");
...
//            System.out.println(LogsT.printDate() + "execute SELECT query in executeQuery is OK");
    ...
    // копия предыдущего метода для update'а, но с возвратом int - колл-во измененых строк
    public static int executeQueryInt(String query) throws SQLException {
        setProperties();
        Connection con = null;
        Statement stmt = null;
        SQLException sqlException = null;
        boolean boolException = false;
        int qntAffectedRows = 0;
        try {
            con = DriverManager.getConnection(url, p);
            con.setAutoCommit(true);
            stmt = con.createStatement();
            qntAffectedRows = stmt.executeUpdate(query);
        } catch (SQLException e) {
            System.out.println("execute SELECT query in executeQuery is failed:");
            e.printStackTrace();
            boolException=true;
...
//                    System.out.println("@@@@@@@ rs.next: md.getScale(i): "+md.getScale(i));
            ...
            System.out.println("execute SELECT query in getResultSet is failed:");
            e.printStackTrace();
        } finally {
            try {
                rs.close();
            } catch(Exception se) {
                System.out.println("closeConnectionWithRs -- RS is NOT closed");
                se.printStackTrace();
            }
            try {
                stmt.close();
    ...
    public static int qntRowsInSelect(String query) throws SQLException {
        ...
        try {
            ...
            System.out.println("execute SELECT query in qntRowsInSelect is failed:");
            e.printStackTrace();
        } finally {
    ...
    public static void closeConnection(Statement stmt, Connection con){
        try {
            ...
            /*can't do anything */
            System.out.println("closeConnection -- STMT is NOT closed");
            se.printStackTrace();
        }
        try {
            con.close();
            ...
            /*can't do anything */
            System.out.println("closeConnection -- CON is NOT closed");
            se.printStackTrace();
        }
